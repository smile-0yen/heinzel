...
<repo>/
  ...
  lib/runtimes/local.sh  starts a process here, via bin/hzl-exec
  ...
  lib/watchdog.sh      hzl_timeout, hzl_signal_tree — the shell half
  go/                  bin/hzl-exec: process control, agent output parsing
  ...
  etc/{pf-travel.conf,sudoers-diag,agent.plist}.in  templates
...
8. Apply the mode's posture when `HEINZEL_POSTURE=1`: remote for `work`, travel
   for `mobile`. Neither installs the write-capable sudo drop-in and both
   remove it (§4.3 of `docs/DESIGN.md`). A failed read-back refuses to start
   the new session. With posture management disabled, leave the OS posture
   unmanaged.
...
| `sudoers.d/heinzel-ticket` | posture, **removal only** |
| `caffeinate` | session |

`heinzel-ticket` carries `!tty_tickets` and a long timeout: a standing,
write-capable sudo window. It was allowed only while posture is `remote` **and**
no session is running, which is not one of the three modes — so **no command
installs it** and every transition removes it, along with any ticket already
granted. `hzl doctor` reports the combination of a live session and a present
ticket file as a defect, not a warning: with nothing here able to write that
file, it can only have come from another tool or from an older build.
...
> `hzl task list` showed a tag the ledger no longer considered part of the task. Tabs
...
Transitions out of `[!]` (`hzl task unblock`, `hzl task done`) are available to humans
...
> `hzl archive` fails; `hzl task block` and `hzl task unblock` exit **5**, having set the
...
no longer `[!]` — `hzl task unblock`, or a person with an editor — moves back into the
backlog at the priority it left with. One-way would strand an unblocked task in a
file no worksheet is built from, which is losing work silently. `hzl task block` and
`hzl task unblock` therefore sweep as part of the command rather than leaving the move
to the next run.
...
| Where do I write this marker? | `ledger_file_of_id`, over the **live** files only. A mutation must never reach the archive: `hzl task done` on an id closed last month says "no such id" rather than closing it twice |
...
| Written by | The agent, at `<workdir>/.heinzel/blocked/<id>.md` — the only place it can write — before it moves the marker. Or a person, or `hzl task take <id> --steps` |
...
| Read by | `hzl report` (the path), `hzl task take <id>` (the whole file), `hzl task list` (which tasks have one) |
| Lifetime | Never deleted by the tool. `hzl task unblock` leaves it and says where it is: the ask was answered, not made wrong, and a later block on the same task writes its own file over it |
...
state file and knows no rule: `GET /api/dashboard` runs `hzl web --json` and
...
`timeout(1)`, and the wall-clock budget is not optional. The contract below is
normative; where it is implemented is not. It is `hzl-exec timeout`
(`go/proc.go`) behind a shell function of the same name.
...
> function — the claim loop, the cleanup rollback, `hzl task done`'s marker and note,
