 ...
 *
 * 判定はここで作らない。`hzl web --json` が返した事実を並べ替えて描くだけで、
...
function renderSession(s) {
  ...
  const rows = [
    ...
    ["エンジン", [
      s.engines.planner_enabled ? `plan: ${s.engines.planner}（${s.engines.planner_model || "既定"}）` : "plan なし",
      s.engines.executor_enabled ? `exec: ${s.engines.executor}（${s.engines.model || "既定"}）` : "exec なし",
      s.engines.reviewer_enabled ? `review: ${s.engines.reviewer}（${s.engines.reviewer_model || "既定"}）` : "review なし",
    ].join(" / ")],
    ["backlog", s.backlog],
  ];
...
/* ── 一つのタスクに何が起きたか ─────────────────────────
 * 行を開くと `hzl task --json <id>` をそのまま読む。ここでも判定は作らない：
 * run が残した marker と、その run が notes に書いた一文を、書いてある通りに出す。
 * marker は「その run がそう置いた」であって「今どうなっているか」ではない——
 * レビューが差し戻した run は done を置いたまま台帳では blocked に戻る。だから
 * 上の `state` と run の行は別のことを言うことがあり、それが正しい。 */
let OPEN_TASK = null;
let DETAIL = null;     // { id, ... } | null（読み込み中）
let DETAIL_ERR = null;

const openable = (id) =>
  id ? ` data-task="${esc(id)}" role="button" tabindex="0"` +
       ` aria-expanded="${OPEN_TASK === id ? "true" : "false"}"` : "";

function detailHTML(id) {
  if (OPEN_TASK !== id) return "";
  if (DETAIL_ERR) {
    return `<div class="detail"><div class="said" data-tone="bad">読めなかった: ${esc(DETAIL_ERR)}</div></div>`;
  }
  if (!DETAIL || DETAIL.id !== id) {
    return `<div class="detail"><div class="said muted">読み込み中…</div></div>`;
  }
  const d = DETAIL;
  const head = `<h3>いま</h3><dl class="kv">
    <dt>状態</dt><dd>${esc(d.state)}</dd>
    <dt>台帳</dt><dd class="mono">${esc(d.file)}</dd>
    ${d.steps ? `<dt>あなたへの手順</dt><dd class="mono">${esc(d.steps)}</dd>` : ""}
    ${d.notes && d.notes.length ? `<dt>メモ</dt><dd>${d.notes.map(esc).join("<br>")}</dd>` : ""}
    </dl>`;
  const runs = !d.runs.length
    ? `<h3>run</h3><div class="said muted">まだどの run も手を付けていない。</div>`
    : `<h3>手を付けた run（${d.runs.length}）</h3>` + d.runs.map((r) => `
      <div class="run">
        <div class="sub">${esc(r.at)} · ${esc(r.outcome)} · <span class="mono">${esc(r.run_id)}</span>${
          r.review ? ` · レビュー ${esc(r.review)}` : ""}</div>
        ${r.note ? `<p class="said">${esc(r.note)}</p>` : ""}
        ${(r.findings || []).map((f) => `<p class="finding">— ${esc(f)}</p>`).join("")}
        <p class="where mono">${esc(r.log)}<br>${esc(r.output)}</p>
      </div>`).join("");
  return `<div class="detail">${head}${runs}
    <p class="where">端末で全文を読む: <span class="mono">hzl task ${esc(d.id)} --full</span>
    ・<span class="mono">hzl task ${esc(d.id)} --logs</span></p></div>`;
}

/* 開いている行は URL に出す。人に「これを見て」と渡せる先が run ではなく
   タスクである方が多い——run id は控えていないが、タスクの番号は控えている。 */
const hashTask = () => (location.hash.match(/^#task=([A-Za-z0-9_-]{1,64})$/) || [])[1] || null;
function syncHash(id) {
  try {
    history.replaceState(null, "", id ? `#task=${id}` : location.pathname + location.search);
  } catch (e) { /* file:// などで history が使えないとき */ }
}

async function openTask(id) {
  if (OPEN_TASK === id) {
    OPEN_TASK = null; DETAIL = null; DETAIL_ERR = null;
    syncHash(null);
    renderAll();
    return;
  }
  ...
  try {
    ...
    if (OPEN_TASK !== id) return;   // その間に別の行が開かれていた
    DETAIL = body;
  } catch (e) {
    if (OPEN_TASK !== id) return;
    DETAIL_ERR = e.message || String(e);
  }
  renderAll();
}

/* renderAll は innerHTML を丸ごと置き換えるので、行ごとに listener は付けない。
   付けても次の更新で消えるし、消えたことは画面から分からない。 */
document.addEventListener("click", (ev) => {
  const el = ev.target.closest("[data-task]");
  if (el) openTask(el.dataset.task);
});
document.addEventListener("keydown", (ev) => {
  if (ev.key !== "Enter" && ev.key !== " ") return;
  const el = ev.target.closest && ev.target.closest("[data-task]");
  if (!el) return;
  ev.preventDefault();
...
function renderDone(s) {
  const rows = s.tasks
    .filter((t) => (t.marker === "x" || t.marker === "X") && t.id)
    .sort((a, b) => String(b.done_at || "").localeCompare(String(a.done_at || "")));
  $("n-done").textContent = rows.length;
  $("done").innerHTML = rows.map((t) => `
    <div class="row"${openable(t.id)}>${chip(mk(t.marker))}
      <div class="nm">${esc(t.text)}</div>
      <div class="foot">
        <span class="sub">${t.done_at ? esc(t.done_at.slice(0, 10)) : "日付なし"} · ${esc(t.workspace)} · ${esc(t.id)}</span>
        <span class="sub">${OPEN_TASK === t.id ? "▾" : "▸"}</span>
      </div></div>${detailHTML(t.id)}`).join("") ||
    `<div class="row"><div class="nm muted">この期間に終わった仕事はない。</div></div>`;
}
...
function renderBlocked(s) {
  ...
  const card = (t) => `
    <div class="card" data-tone="late"${openable(t.id)}>
      ...
      <div class="act">${esc(t.workspace)} · ${t.blocked_at ? esc(t.blocked_at.slice(0, 10)) : "日付なし"}
        ${t.id ? `· <span class="mono">hzl task take ${esc(t.id)}</span>` : ""}</div>
    </div>${detailHTML(t.id)}`;
...
function renderQueue(s) {
  ...
  $("queue").innerHTML = rows.map((t, i) => {
    ...
    return `<div class="row"${openable(t.id)}>${chip(ok ? mk(t.marker) : { tone: "bad", glyph: "⛔", text: "行き先なし" })}
      <div class="nm">${esc(t.text)}</div>
      <div class="foot">
        <span class="sub">P${t.priority} · ${esc(t.workspace)}${ok ? "" : "（このセッションにはない）"} · ${esc((t.roles || []).join(","))} · ${esc(t.id || "id は run が採番")}</span>
        <span class="sub">${i === 0 && ok ? "次に着手" : ""}</span>
      </div></div>${detailHTML(t.id)}`;
  }).join("") ||
...
window.addEventListener("hashchange", () => {
  const id = hashTask();
  if (id && id !== OPEN_TASK) openTask(id);
});

renderFreshness(null);
load().then(() => {
  // 開いたままの URL で来た人には、一覧が出るのと同じ更新でその中身も出す。
  const id = hashTask();
  if (id) openTask(id);
});
